"""
Script per verificare le valutazioni per un sottoinsieme selezionato di strutture.

Questo script verifica che le valutazioni di alcune strutture specifiche nel database 
...
# Lista di strutture da verificare (nomi esatti dal database)
SELECTED_FACILITIES = [
    'Ospedale di Gubbio - Gualdo Tadino',   # Aggiornato manualmente
    'IDI - Istituto Dermopatico dell\'Immacolata',  # Aggiornato precedentemente
    'Ospedale Niguarda',          # Prima riga del CSV
    'Ospedale San Carlo Borromeo', # Seconda riga del CSV
    'Ospedale Umberto Parini',    # A metà CSV
    'Ospedale G. Vietri',         # Verso la fine del CSV
    'Ospedale San Giovanni di Dio' # Ultima riga del CSV
]
...
def verify_selected_facilities(csv_file, facility_names):
    """
    Verifica le valutazioni solo per le strutture specificate
    
    Args:
        csv_file: Il percorso del file CSV
        facility_names: Lista di nomi di strutture da verificare
    ...
    for facility_name in facility_names:
        ...
        if not facility:
            ...
            total_stats['facilities_not_found'].append(facility_name)
            continue
...
def print_discrepancies(stats):
    ...
    # Strutture non trovate
    if stats['facilities_not_found']:
        print("\n--- STRUTTURE NON TROVATE NEL DATABASE ---")
        for facility_name in stats['facilities_not_found']:
...
def print_stats(stats):
    ...
    print(f"Valutazioni corrispondenti: {stats['matching_ratings']} ({stats['matching_ratings']/stats['total_ratings']*100:.2f}% se corretto)")
    print(f"Valutazioni non corrispondenti: {stats['mismatched_ratings']} ({stats['mismatched_ratings']/stats['total_ratings']*100:.2f}% se diverso)")
    print(f"Valutazioni mancanti nel database: {stats['missing_in_db']} ({stats['missing_in_db']/stats['total_ratings']*100:.2f}% se mancante)")
    print(f"Valutazioni mancanti nel CSV: {stats['missing_in_csv']} ({stats['missing_in_csv']/stats['total_ratings']*100:.2f}% se in più)")

if __name__ == "__main__":
    csv_file = "./attached_assets/medical_facilities_full_ratings.csv"
    print(f"Verifica delle valutazioni per strutture selezionate dal file {csv_file}...")
    
    # Eseguo la verifica
    stats = verify_selected_facilities(csv_file, SELECTED_FACILITIES)
